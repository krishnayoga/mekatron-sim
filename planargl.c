...
{
	glPushMatrix();
    glTranslatef(0.025, 0.2, 0.0);
    glPushMatrix();
    	model_box(0.005, 0.6, 0.05, gray8, gray7, gray6,1);
    glPopMatrix();
    glTranslatef(-0.05, 0.0, 0.0);
    glPushMatrix();
    	model_box(0.005, 0.6, 0.05, gray8, gray7, gray6,1);
    glPopMatrix();
    glTranslatef(0.025, 0.0, 0.02);
    glPushMatrix();
    	model_box(0.05, 0.6, 0.005, blue1, blue1, blue1,1);
    glPopMatrix();
    glTranslatef(0.0, 0.28, 0.02);
    glPushMatrix();
    	glTranslatef(0.0, objTrans, 0.0);
    	model_box(0.03, 0.03, 0.03, yellow5, yellow5, yellow5,1);
    glPopMatrix();
	
	
    glPushMatrix();
    	glTranslatef(0.0, -0.62, -0.05);
    	model_box(0.03, 0.05, 0.02, gray8, gray7, gray6,1);
//    glTranslatef(Xoffset, Yoffset, Zoffset);
    glPopMatrix();
    // Draw base
    //model_cylinder(obj, 0.03, 0.03, Link1, 2, blue1, yellow2);
    // Menuju joint-1
	glTranslatef(0, -0.62, -0.04);
...
//    glPushMatrix();
//    	model_box(0.003,0.05,0.1,gray8,gray7,gray6,1);
//    glPopMatrix();
//    glPopMatrix();
//  glPopMatrix();
//  glPushMatrix();
  ...
  if (gerak == 1) {
    ...
    while (k <= N*PERIOD) {
      ...
      if (counter%((N/divider)) == 0) {
      	convert3();
        printf("%.4f %.4f %.4f %.4f %.4f %.4f %.4f\n",xx,yy,z, q1*RTD-90, q2*RTD,q3*RTD,q4*RTD);
...
//        if (sudut3 < 0) sudut3 += 360;
        ...
        XOR = kirimsudut1_0 ^ kirimsudut1_a ^ kirimsudut1_b ^ kirimsudut2_0 ^ kirimsudut2_a ^ kirimsudut2_b ^ kirimsudut3_0 ^ kirimsudut3_a ^ kirimsudut3_b ^ sudut4;
        
        write(fd,&header,sizeof(header));//header
        write(fd,&XOR,sizeof(XOR));//data XOR
...
void keyboard(unsigned char key, int i, int j)
{
	 ...
	 /* Joint Control */
	 case ESCkey: exit(1);
     case '1' : x_final = L1+L3; y_final = 0; z_final = L2; q4=0*DTR; gerak = 1; break;
     case '2' : z_final = z_final+0.02; gerak=1; break;
     case '3' : z_final = z_final-0.02; gerak=1; break;
	 case '4' : y_final = y_final+0.02; gerak=1; break;
	 case '5' : y_final = y_final-0.02; gerak=1; break;
	 case '6' : x_final =x_final+0.02; gerak = 1; break;
	 case '7' : x_final =x_final-0.02; gerak = 1; break;
	 case 'm' : q4=0*DTR; gerak=1;gerak1=2; break;
	 case 'n' : q4=180*DTR; gerak=1; gerak1=2; break;
 	//Kamera	
...
int main(int argc, char** argv)
{
 // Initialize GLUT
   ...
   /* Open a window */  
   window = glutCreateWindow ("4 DOF Taskspace");
